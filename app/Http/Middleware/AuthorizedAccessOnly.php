    ...
    public function handle(Request $request, Closure $next)
    {
        if (Auth::check()) {
            ...
            if (empty($jabatan_id)) {
                ...
                $access = PositionAccess::whereIn('access_id', [6,8,9])->where('jabatan_id', $jabatan_id)->get();
                if ($access->count() > 0) {
                    return $next($request);
                } else {
                    $personal_access = PersonalAccess::whereIn('access_id',[6,8,9])->where('pns_id', Auth::user()->v2Profile->id)->get();
